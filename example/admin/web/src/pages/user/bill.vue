...
export default {
    ...
    setup() {
        const visibleStorage = '_table_visible_019'
        ...
        const state = reactive({
            ...
            checkboxList: [] as any,
            conf: {
                basic: {indexURL: '/user/bill/index', updateURL: '/user/bill/update', updateKey: 'id'},
                search: [{label: '管理', field: 'admin_name', type: 'text'}, {
                    label: '用户',
                    field: 'username',
                    type: 'text'
                }, {label: '标题', field: 'name', type: 'text'}, {
                    label: '类型',
                    field: 'type',
                    type: 'selectNumber',
                    data: []
                ...
                visibleColumns: localStorage.getItem(visibleStorage) ?
                    ...
                    ['id', 'admin_name', 'username', 'name', 'type', 'balance', 'money', 'created_at', 'options'],
                columns: [{
                    label: '主键',
                    field: 'id',
                    name: 'id',
                    ...
                    items: [{
                        label: '编辑',
                        url: '/user/bill/update',
                        color: 'primary',
                        type: 'update',
                        tips: '',
                        isShow: '',
                        items: [{label: '时间', field: 'created_at', type: 'datePicker'}]
                    }]
                }]
            }
        })
        ...
        onMounted(() => {
            // 请求账单类型列表
            api.post('/user/bill/types', {}, {showLoading: false} as any).then((res: any) => {
                state.conf.columns[4].data = res
                state.conf.search[3].data = res
            })
