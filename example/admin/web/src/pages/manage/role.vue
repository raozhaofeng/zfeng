...
export default {
    ...
    setup() {
        const visibleStorage = '_table_visible_004'
        const dialogFormRef = ref(null) as any;
        const state = reactive({
            params: {
                pagination: {sortBy: 'name', descending: true, page: 0, rowsPerPage: 10}
            } as any,
            dialogForm: {} as any,
            rows: [] as any,
            dynamicData: {} as any,
            ...
            checkboxList: [] as any,
            conf: {
                basic: {indexURL: '/role/index', updateURL: '/role/update', updateKey: 'name'},
                search: [{label: '名称', field: 'name', type: 'text'}, {label: '描述', field: 'description', type: 'text'}],
                tools: [{
                    label: '新增',
                    url: '/role/create',
                    color: 'primary',
                ...
                visibleColumns: localStorage.getItem(visibleStorage) ?
                    ...
                    ['name', 'permissions', 'description', 'updated_at', 'options'],
                columns: [{
                    label: '名称',
                    field: 'name',
                    name: 'name',
                    ...
                    items: [{
                        ...
                        tips: '',
                        isShow: '',
                        items: [{label: '权限', field: 'authority', type: 'checkbox', data: []}, {
                            label: '描述',
                            field: 'description',
                            type: 'text'
                        }]
        ...
        onMounted(() => {
            api.post('/role/permissions', {}, {showLoading: false} as any).then((res: any) => {
                state.dynamicData['authority'] = res
            })
